	...
	<div class="relative bg-white flex flex-col items-center w-full">
		...
		<div class="relative w-full bg-white">
			<div
				...
				style={isEnd
					? `position: absolute; top: ${introText.top + scrollY}px; left: ${introText.left}px; right: ${introText.right}px; bottom: ${introText.bottom + scrollY}px; z-index: 50; width: 100%;`
					: ''}
			>
				{#if scrollY < introCanvasHeight + viewportHeight / 1.5}
					<div class="grid text-bb-black gap-[27px] text-content animate-fade-in text-center">
						<h1 class="animate-gradient">BerdeBox</h1>
						<h5 class="text-bb-dark-green/[.8]">
							Receive packages securely from anywhere, anytime
						</h5>
			...
			<div class="relative z-40">
				...
				<ImageSequence
					...
					frameCount={framesIntro}
					bind:isEnd
					initialY={0}
					{scrollY}
					duration={2 * introCanvasHeight}
					images={intro}
				/>
			</div>
		</div>
		...
		<div class="flex flex-row md:m-40 m-10 bg-white items-center justify-center text-bb-black">
			<div class="flex flex-col items-center gap-2">
				<h5>Features</h5>
				<h3 class="w-[90%] md:w-[60%] text-center">Your personal delivery assistant</h3>
			</div>
		</div>
		...
		<div id="features" class="relative container mx-auto py-10 z-40">
			<div class="grid gap-8 mb-10">
				<!-- FEATURE CARDS -->
				<div class="grid grid-cols-1 md:grid-cols-3 gap-8 mx-10">
					<!-- each feature card -->
					<FeatureCard
				...
				>
					<div
						class="max-w-lg text-left text-bb-black flex flex-col space-y-4 ml-auto pl-16 md:pl-0 md:pr-16"
					>
						<h4 class="leading-snug">Your packages, safe and secure with auto-locking</h4>
		...
		<div id="modules" class="flex flex-row m-40 bg-white items-center justify-center text-bb-black">
			<div class="flex flex-col items-center gap-2">
				<h5>Modules</h5>
				<h3 class="w-[90%] md:w-[60%] text-center">Plug and play modules</h3>
			</div>
		</div>

		<!-- CASHBOX MODULE -->
		<div class="relative w-full h-[70vh]" bind:this={cashboxElement}>
			<div class="absolute w-full md:max-w-[60%] max-md:-mt-[250px]">
				<ImageSequence
					bind:canvas={cashboxCanvasElement}
					frameCount={framesModules}
					initialY={cashboxInitialY}
					{scrollY}
					duration={cashboxCanvasHeight}
					images={cashbox}
				/>
			</div>
			<div
				class="absolute w-full h-[50vh] items-center grid max-md:grid-rows-2 md:grid-cols-2 z-50 text-bb-black"
			>
				<div />
				<div class="flex flex-col gap-8 relative p-[20%] max-md:pt-[200px] md:w-[90%]">
					<h3>Cashbox</h3>
					<p>
						Release money remotely with full control - attach multiple cashboxes to transact with
		...
		<div class="relative w-full h-[70vh]" bind:this={uvboxElement}>
			<div class="absolute w-full md:max-w-[60%] max-md:-mt-[200px]">
				<ImageSequence
					bind:canvas={uvboxCanvasElement}
					frameCount={framesModules}
					initialY={uvboxInitialY}
					{scrollY}
					duration={uvboxCanvasHeight}
					images={uvlight}
				/>
			</div>
			<div
			...
			>
				...
				<div class="flex flex-col gap-8 relative p-[20%] max-md:pt-[200px] md:w-[90%]">
					<h3>UV Lightbox</h3>
					<p>
						Automatically disinfects the contents of your BerdeBox after a successful transaction.
					</p>
				</div>
			</div>
		</div>
		...
		<div class="relative w-full h-[70vh]" bind:this={solarElement}>
			<div class="absolute w-full md:max-w-[60%] max-md:-mt-[200px]">
				<ImageSequence
					bind:canvas={solarCanvasElement}
					frameCount={framesModules}
					initialY={solarInitialY}
					{scrollY}
					duration={solarCanvasHeight}
					images={solarPanel}
				/>
			</div>
			<div
		...
		>
			<!-- Text -->
			<div>
				<div class="flex flex-col p-20 text-bb-black gap-16 md:w-[90%] md:ml-5 xl:ml-10">
					<div class="grid items-center justify-center gap-4">
						<h4 class="leading-relaxed">“Kuya, iwan mo nalang sa BerdeBox.”</h4>
						<p class="">
							Take control of your online deliveries. Pre-order your BerdeBox today and get a free
							cashbox module.
						</p>
					</div>
					...
					<div class="flex gap-4">
						<button
							class="w-[160px] border border-bb-light-green bg-gradient-to-tr from-bb-light-green p-4 hover:text-white hover:bg-bb-green shadow-[0px 14px 50px 0px #557760] flex flex-col items-center rounded-[15px] duration-300 ease-in-out"
						>
							Pre-order Now
						</button>
						<a href="/app" class="button-link">
							<button
								class=" border border-bb-light-green p-4 hover:text-white hover:bg-bb-green shadow-[0px 14px 50px 0px #557760] flex flex-col items-center rounded-[15px] duration-300 ease-in-out"
							>
								Try BerdeBox
							</button>
						</a>
			...
			<!-- Image -->
			<img src="/mockups/7.png" alt="berdebox still" class="h-full aspect-auto" />
		</div>
	</div>
</body>
<Footer />
